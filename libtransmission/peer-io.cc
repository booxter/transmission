...
auto tr_peerIo::queued_outgoing_bytes() const noexcept -> QueuedOutgoingBytes
{
    auto queued = QueuedOutgoingBytes{};

    for (auto const& [n_bytes, is_piece_data] : outbuf_info_)
    {
        if (is_piece_data)
        {
            queued.piece_bytes += n_bytes;
        }
        else
        {
            queued.protocol_bytes += n_bytes;
        }
    }
