...
{
    EXPECT_TRUE(tr_isFilePriority(TR_PRI_LOW));
    EXPECT_TRUE(tr_isFilePriority(TR_PRI_NORMAL));
    EXPECT_TRUE(tr_isFilePriority(TR_PRI_HIGH));

    EXPECT_TRUE(tr_isTorrentPriority(TR_TOR_PRI_LOW));
    EXPECT_TRUE(tr_isTorrentPriority(TR_TOR_PRI_NORMAL));
    EXPECT_TRUE(tr_isTorrentPriority(TR_TOR_PRI_HIGH));
    EXPECT_TRUE(tr_isTorrentPriority(TR_TOR_PRI_FORCE));

    auto* ctor = tr_ctorNew(session_);
    ASSERT_NE(nullptr, ctor);
    EXPECT_EQ(TR_TOR_PRI_NORMAL, tr_ctorGetBandwidthPriority(ctor));

    tr_ctorSetBandwidthPriority(ctor, TR_TOR_PRI_FORCE);
    EXPECT_EQ(TR_TOR_PRI_FORCE, tr_ctorGetBandwidthPriority(ctor));
    tr_ctorFree(ctor);
    ...
    tr_torrentSetPriority(tor, TR_TOR_PRI_FORCE);
